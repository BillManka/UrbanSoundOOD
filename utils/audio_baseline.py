if __name__ == '__main__':
    ...
    parser.add_argument('--model', '-m', type=str, default='allconv',
                        choices=['allconv', 'wrn', 'melspecnet', 'pretrained_resnet'], help='Choose architecture.')
    ...
    def make_filelist(dir_path):
        ...
        for class_dir in dir_path.iterdir():
            for wav_path in class_dir.iterdir():
                filelist.append((wav_path, class_dir.name))
        return filelist
    ...
    #     range(len(usd)),
    #     usd.targets,
    #     stratify=usd.targets,
    #     test_size=test_size,
    #     random_state=seed
    # )

    # train_split = Subset(usd, train_indices)
    # test_split = Subset(usd, test_indices)

    train_loader = DataLoader(train_split, batch_size=args.batch_size, shuffle=True, pin_memory=True)
    test_loader = DataLoader(test_split, batch_size=args.batch_size, pin_memory=True)
